...
BOOL CNotifyIcon::Modify(LPCTSTR szTip)  // private method
{
...
void CNotifyIcon::Dispatch(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (WM_RBUTTONDOWN == lParam  || WM_RBUTTONDBLCLK == lParam)
	{
		OnNIContextMenu(hWnd, (UINT)wParam);
	}
    
	if (WM_LBUTTONDOWN == lParam  || WM_LBUTTONDBLCLK == lParam )
	{
		theApp.IsSettingsDlgVisible() ? FALSE : theApp.ShowSettingsDlg();
